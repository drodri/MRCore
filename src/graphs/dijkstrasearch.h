#ifndef __DIJKSTRA_SEARCH_H
#define __DIJKSTRA_SEARCH_H
...
class DijkstraSearcher: public Searcher<N,E>{
	...
	class AuxObject{
		public:
			AuxObject(int s, E cost){status=s; costToCome=cost; comingFrom=NULL;}
			NODE* comingFrom;
			E costToCome;
			int status;
	};
	...
	public:
		DijkstraSearcher(const E& z, const E& i):ZERO_COST(z),INFINITE_COST(i){this->graph=NULL;}
		DijkstraSearcher(GRAPH* g, const E& z, const E& i):ZERO_COST(z),INFINITE_COST(i){this->graph=g;}
		virtual ~DijkstraSearcher(){}

		bool findPath(NODE* initNode, NODE* goalNode){
			E costToCome;
			return findPath(initNode, goalNode, costToCome);
		}
		...
		bool findPath(NODE* initNode, NODE* goalNode, E& costToCome){
			...
			obj->comingFrom=NULL; // for reconstructing this->path
			...
			while(!searchQueue.empty()){
				...
				if (currentNode==NULL){
					cout << "DijkstraSearch: CurrentNode = NULL, this should never happen" << endl;
					continue;
				}
		...
		void initialize()
		{
			...
			for (itn=this->graph->nodes.begin();itn!=this->graph->nodes.end();itn++){
				if (*itn != NULL) {
					(*itn)->object = new AuxObject(NON_VISITED, INFINITE_COST); //NOT VISITED, costToCome=INIFINITE_COST, coming_from=NULL
				}
			}
	...
	private:
		...
		void djInsert(vector<NODE*> & queue, NODE* nd){
			...
			if (queue.size()>0){
				...
				if (obj->costToCome <= objinit->costToCome){
					queue.insert(queue.begin(), nd);
					return;
				}
				// at the end
				AuxObject* objend = (AuxObject*)queue.back()->object;
				if (obj->costToCome >= objend->costToCome){
					queue.push_back(nd);
					return;
				}
			...
			for (it=queue.begin(); it!= queue.end(); it++){
				...
				if(obj->costToCome <= current->costToCome){
	...
	*/
	typename vector<NODE*>::iterator findNode(vector<NODE*> & queue, NODE* nd){	
			...
			while ( nd != queue[middle]){
				...
				//stop condition
				if (middleObj1->costToCome == obj->costToCome || middleObj2->costToCome == obj->costToCome ) break; //end while
				
				if ( middleObj1->costToCome > obj->costToCome ){ // it is before
					end = middle;
					middle = (end + init) / 2;
				} else{ //it is after
					init = middle;
